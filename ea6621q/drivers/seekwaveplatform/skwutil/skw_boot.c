...
static int seekwave_boot_probe(struct  platform_device *pdev)
{
	...
	if (!io_bus) {
		if (usb_chip_enable_gpio >= 0) {
			gpio_free(usb_chip_enable_gpio);
			ret = devm_gpio_request_one(&pdev->dev, usb_chip_enable_gpio, GPIOF_OUT_INIT_HIGH,"USB_CHIP_EN");
			printk("request gpio %d ret = %d\n", usb_chip_enable_gpio, ret);
		}
